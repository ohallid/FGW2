function [criteria_array_0, delta_w_CAPE] = triggering_criteria_CAPE_diffs()
% 289 X 1001
...
criteria_array_diffs_0    = zeros(64,101);
criteria_array_diffs_1    = zeros(64,101);
a    = 1 / 3;
b    = 2 / 3;
c    = 3 / 3;
%
...
for k = 1:64
    ...
    for l = 1:101
        if delta_w_CIN(k,l) > 0 && delta_b_CIN(k,l) < 0
            criteria_array_diffs_0(k,l) = a;
        end
        if delta_w_CIN(k,l) < 0 && delta_b_CIN(k,l) > 0
            criteria_array_diffs_0(k,l) = b; 
        end
        if delta_w_CIN(k,l) > 0 && delta_b_CIN(k,l) > 0
            criteria_array_diffs_0(k,l) = c;
        end
        %%%%%%%% CIN long
        if w_CIN0(k,l) > 0 && b_CIN0(k,l) < 0
        ...
        %%%%%%%%
         if delta_w_CAPE(k,l) > 0.0 && delta_b_CAPE(k,l) < 0.0
            criteria_array_diffs_1(k,l) = a;
        end
        if delta_w_CAPE(k,l) < 0.0 && delta_b_CAPE(k,l) > 0.0
            criteria_array_diffs_1(k,l) = b; 
        end
        if delta_w_CAPE(k,l) > 0.0 && delta_b_CAPE(k,l) > 0.0
            criteria_array_diffs_1(k,l) = c;
        end
    end
end
% 
...
criteria_array_diffs_0 = flipud(criteria_array_diffs_0);
criteria_array_diffs_1 = flipud(criteria_array_diffs_1);
...
% plot 4 - CIN diffs only
figure(4)
imagesc(criteria_array_diffs_0);
xlim([0 100])
ylim([0 64])
...
% plot 5 - CAPE diffs only
figure(5)
imagesc(criteria_array_diffs_1);
xlim([0 100])
ylim([0 64])
