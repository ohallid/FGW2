function [ c3, Nfound3 ] = NR33(HL_bar , Ht_bar, N1, N2)
...
Hkn        = ( 0.25 / ratiopp1 / ratiopp1 - N1 * N1 .* x .* x ) .^ 0.5;
Hknp       = ( 0.25 / ratiopp2 / ratiopp2 - N2 * N2 .* x .* x ) .^ 0.5;
fx         = Hkn ./ Hknp + ( 1/ratiopp1 - 1/ratiopp2 ) * tanh( Hkn*ratio ) ./ Hknp - tanh ( Hkn * ratio ) .* coth( Hknp * (ratio - 1 ) );
index      = 0;
for n=1:N
...
for n=1:10
    ...
    f       = Hkn ./ Hknp + ( 1/ratiopp1 - 1/ratiopp2 ) * tanh( Hkn*ratio ) ./ Hknp - tanh ( Hkn * ratio ) .* coth ( Hknp * (ratio - 1 ) );
    
    dfdx    = - Hkn ./ ( Hknp .^ 2 ) .* dHknpdx + 1 ./ Hknp .* dHkndx...
              - ( 1/ratiopp1 - 1/ratiopp2 ) *         tanh( Hkn * ratio )                       ./ Hknp ./ Hknp .* dHknpdx...
              + ( 1/ratiopp1 - 1/ratiopp2 ) * ratio * sech( Hkn * ratio ).* sech( Hkn * ratio ) ./ Hknp         .* dHkndx ...
              + tanh ( Hkn * ratio         ) .* ( ( sinh( Hknp * (ratio - 1 ) )   .^-2 ).* dHknpdx * (ratio - 1 ))...
              - coth ( Hknp * (ratio - 1 ) ) .* ( ( sech   ( Hkn  *  ratio      ) .^ 2 ).* dHkndx  *  ratio      );
