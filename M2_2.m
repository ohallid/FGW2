function [mm] = M2_2(bm,HL_bar,c,A,fac, s, x, x_0, z, t, T, Ht_bar, sigma, N1, N2 )
...
x1           = c * c * F2_2(x, x_0, s * HL_bar, t  , sigma) / ( c*c - s * s / HL_bar / HL_bar) ;  
x2           = c * c * F2_2(x, x_0, c * HL_bar, t  , sigma) /  c / ( c - s / HL_bar );
x3           = c * c * F2_2(x, x_0,-c * HL_bar, t  , sigma) /  c / ( c + s / HL_bar );
x2p          = c * c * F2_2(x, x_0, c * HL_bar, t-T, sigma) /  c / ( c - s / HL_bar );
x3p          = c * c * F2_2(x, x_0,-c * HL_bar, t-T, sigma) /  c / ( c + s / HL_bar );
...
phi(region1) = A / sigma       * sin ( Hkn1  .* z(region1) / HL_bar         ) .* exp( 1 .* z(region1) / 2 / Ht_bar * ratiop1);
